...
def upload_file():
    ...
    try:
        ...
        # Pipeline migration: reset if keys still contain date suffix (old format)
        # Forces reprocess with new Borrower|Lender key (no date)
        existing_pl = stored.get("pipeline_rows", [])
        if existing_pl and "|" in existing_pl[0].get("borrower","") + "|" + str(len(existing_pl)):
            pass  # already migrated
        if existing_pl:
            sample_key = f"{existing_pl[0].get('borrower','')}|{existing_pl[0].get('lender','')}|{existing_pl[0].get('date_of_lead','')}"
            # If old rows exist with date in key, wipe and reprocess
            stored["pipeline_rows"] = []
