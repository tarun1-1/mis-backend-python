...
def normalize_jarvis_rows(rows):
    ...
    for r in rows:
        ...
        # Map Jarvis AI_Tag → primary_dispo (capitalize to match Flexy style)
...
def upload_file():
    ...
    try:
        ...
        # One-time migration: if blended data missing, reset both dialer AND handoff dates
        # so handoff mobiles get re-extracted and blended matching works correctly
        if not stored.get("dialer_blended_by_day"):
        ...
        # Normalize Flexy agent names + merge Jarvis rows
        for r in d_rows:
            if r.get("user"):
