...
def aggregate_ai(rows, processed_dates):
    by_day = {}
    added = skipped = 0
    for r in rows:
        date = excel_to_date(r.get("Call Date"))
    ...
    return result, set(result.keys()), added, skipped


def aggregate_handoff(rows, processed_dates):
    by_day = {}
    added = skipped = 0
    for r in rows:
        date  = excel_to_date(r.get("Call Date"))
    ...
    return by_day, set(by_day.keys()), added, skipped


def aggregate_dialer_manual(rows, processed_dates):
    """Process only callmode=manual rows for Stage 2B."""
    by_day = {}
    added = skipped = 0
    for r in rows:
        callmode = ss(r.get("callmode", "")).lower()
        ...
        # talk_time in Dialer is fractional days → convert to seconds
        ...
        if date not in by_day:
            by_day[date] = {
                "total_dials": 0, "connected": 0, "talk_time_sec": 0.0,
                "dispositions": {}, "agents": {}
            }
        d = by_day[date]
        d["total_dials"]   += 1
        ...
        if agent not in d["agents"]:
            d["agents"][agent] = {
                "dials": 0, "connected": 0, "talk_time_sec": 0.0, "dispositions": {}
            }
        a = d["agents"][agent]
        a["dials"]        += 1
...
def _sum_dialer(by_day, dates):
    r = {"total_dials": 0, "connected": 0, "talk_time_sec": 0.0,
         "dispositions": {}, "agents": {}}
    for d in dates:
        day = by_day.get(d, {})
        r["total_dials"]   += day.get("total_dials", 0)
...
def upload_file():
    ...
    try:
        ...
        stored = load_json(DATA_FILE) or {
            "processed_dates":        {"ai_dump": [], "handoff": [], "dialer_manual": []},
            "pipeline_rows":          [],
            "ai_by_day":              {},
            "handoff_by_day":         {},
        ...
        pd_dm   = set(stored["processed_dates"].get("dialer_manual", []))

        # Aggregate each sheet
        ai_new,  ai_dates,  ai_add,  ai_skip  = aggregate_ai(ai_rows,             pd_ai)
        h_new,   h_dates,   h_add,   h_skip   = aggregate_handoff(h_rows,         pd_h)
        dm_new,  dm_dates,  dm_add,  dm_skip  = aggregate_dialer_manual(d_rows,   pd_dm)
        ...
        stored["processed_dates"]["dialer_manual"]  = sorted(pd_dm | dm_dates)
