...
# Legacy file paths (used as fallback if PostgreSQL unavailable)
...
def get_db():
    ...
    if not HAS_PSYCOPG2:
        return None
    # Support both single DATABASE_URL and individual params
...
def db_get(key, default=None):
    ...
    for attempt in range(2):
        ...
        try:
            ...
            _db_conn = None  # Force reconnect on next attempt
    # Fallback to file
...
def _supabase_keepalive():
    """Background thread: ping Supabase every 20 min to prevent free-tier pause."""
    while True:
        ...
        try:
            ...
            if conn:
                ...
                print("Supabase keepalive ping OK")
        except Exception as e:
...
def _start_keepalive():
    t = threading.Thread(target=_supabase_keepalive, daemon=True)
    t.start()
...
def db_get_monthly_sets(prefix):
    """Load all monthly sets for a given prefix. Returns {YYYY-MM: set}."""
    conn = get_db()
    result = {}
    if conn:
        try:
...
def normalize_mobile(raw):
    """Strip +91 / 91 prefix, spaces, dashes → bare 10-digit number for comparison."""
...
def excel_to_date(val):
    ...
    Accepts: DD/MM/YYYY, DD-MM-YYYY, YYYY-MM-DD, DD Mon YYYY, DD-Mon-YYYY,
    MM/DD/YYYY, YYYY/MM/DD, datetime strings with time component.
    Recommended format for uploads: DD/MM/YYYY or DD-MM-YYYY."""
    if val is None:
        return None
    ...
    if isinstance(val, str):
        s = val.strip()
        # Strip time component if present (e.g. "2026-07-13 00:00:00")
        if ' ' in s and ':' in s:
            s = s.split(' ')[0]
        for fmt in [
            "%d/%m/%Y", "%d-%m-%Y", "%Y-%m-%d",
...
def _norm(n):
    """Normalize sheet name: lowercase, remove spaces/underscores/hyphens."""
    return n.lower().replace(" ","").replace("_","").replace("-","")

def find_sheet_name(wb, candidates):
    """Return actual sheet name from wb matching any candidate.
    Tries exact → case-insensitive → normalized fuzzy (strips spaces/_/-)."""
    for c in candidates:
    ...
    for c in candidates:
        nc = _norm(c)
        for s in wb.sheetnames:
            if nc in _norm(s) or _norm(s) in nc:
                return s
    return None

def read_sheet(wb, sheet_name, cols=None):
    """Read sheet → list of dicts (column names stripped of whitespace).
    If cols is provided, only those columns are included (reduces memory)."""
    ...
    if cols:
        col_idx = {h: i for i, h in enumerate(header) if h in cols}
    else:
    ...
    for r in rows[header_idx + 1:]:
        if all(c is None for c in r):
            continue
        row = {h: r[i] for h, i in col_idx.items() if i < len(r)}
        result.append(row)
    return result
...
def stream_sheet(wb, sheet_name, cols=None):
    ...
    Use for very large sheets (AI_Dump) to avoid OOM on free-tier servers."""
    if sheet_name not in wb.sheetnames:
        return
    ws = wb[sheet_name]
    header = None
...
def normalize_agent(name):
    """Normalize agent name: title case, collapse repeated words, strip whitespace."""
    if not name:
        return ""
    parts = str(name).strip().split()
    # Remove consecutive duplicate words (e.g. "Dipanshu Dipanshu" -> "Dipanshu")
...
def normalize_jarvis_rows(rows):
    """Convert Jarvis_Dump rows to Flexy-compatible format for aggregate_dialer.
    Maps: Number->customer_cid, Agent Name->user, Status->dialed_status,
          Duration(s)->talk_time(days), Started At->init_time, AI_Tag->primary_dispo.
    Keeps only outbound calls. Skips inbound."""
    normalized = []
    for r in rows:
        direction = ss(r.get("Direction", "")).lower()
        if direction == "inbound":
            continue  # exclude inbound calls
        ...
        normalized.append({
            "customer_cid":  cid,
            "user":          agent,
            "callmode":      "manual",   # Jarvis treated as manual
            "init_time":     date,
            "talk_time":     dur_s / 86400.0,  # seconds → Excel day fraction
...
def aggregate_dialer(rows, processed_dates, handoff_set=None,
    ...
    """Route dialer rows: if customer_cid is in handoff mobile set → Stage 2A (AI Handoff Tele).
    Otherwise → Stage 2B (Direct Tele). Callmode is irrelevant for routing."""
    blended_by_day = {}
    manual_by_day  = {}
    added = skipped = 0
    ...
    for r in rows:
        ...
        agent    = ss(r.get("user", ""))
        # Skip system-generated rows (no user assigned)
        if not agent:
            added += 1
            continue
        ...
        # Route: any call on a handoff mobile → Stage 2A; everything else → Stage 2B
...
def process_pipeline(rows, existing_rows):
    """Update pipeline using borrower|lender as key (no date).
    When the same borrower+lender appears multiple times (status change),
    keep the latest row — reflects current status only.
    Returns (updated_list, added, updated, fuzzy_warnings)."""
    pipeline = {}
    for row in existing_rows:
        key = f"{row.get('borrower','')}|{row.get('lender','')}"
        pipeline[key] = row
...
def upload_file():
    ...
    try:
        ...
        sn_pipe   = find_sheet_name(wb, ["Login_Pipeline", "Login Pipeline", "Login Pipeline", "Pipeline"])

        missing_hard = [n for n, s in [
            ("AI_Dump", sn_ai), ("Handoff_Leads", sn_hand)
        ] if not s]
        # Flexy OR Jarvis — at least one dialer must exist
        ...
        # Load smaller sheets normally; stream AI_Dump to avoid OOM
        ...
        # Validate all non-AI sheets before processing
        ...
        if sn_pipe and p_rows:
            _, pw = validate_pipeline(p_rows)
            all_warnings.extend([f"[Login Pipeline] {w}" for w in pw])
        if pipeline_missing:
            all_warnings.append(_w("Login Pipeline", "-", "Sheet missing — pipeline not updated"))

        if all_errors:
            wb.close(); os.remove(tmp)
            return jsonify({"error": "Validation failed", "validationErrors": all_errors,
                            "warnings": all_warnings}), 422

        # Stream AI_Dump — never loads all rows into memory
        ai_stream = stream_sheet(wb, sn_ai,
                                 cols=["Customer Number","Call Date","L1 Disposition","Call Duration","Batch"])                     if sn_ai else iter([])
        ...
        # Migration v2: force full dialer+handoff reprocess
        # jarvis_v2 flag ensures this runs once after July date corruption
        if not stored.get("jarvis_v2"):
            ...
            stored["jarvis_processed"]       = True
            stored["jarvis_v2"]              = True

        # Pipeline migration complete — no reset needed

        # Safety check: if stored is a fresh empty dict but Supabase is connected,
        # verify this isn't a silent connection failure masking existing data
        if not stored.get("processed_dates", {}).get("ai_dump"):
            # Either truly first upload or Supabase returned empty — safe to proceed
            pass
        ...
        # Aggregate AI — streamed row by row (no full list in memory)
        ...
        wb.close(); os.remove(tmp); gc.collect()  # close workbook after AI stream done
        ...
        # Tag Flexy rows with source, normalize agent names
        for r in d_rows:
            r.setdefault("_source", "flexy")
            if r.get("user"):
                r["user"] = normalize_agent(ss(r["user"]))
        jarvis_normalized = normalize_jarvis_rows(jarvis_rows)
        ...
        b_new, dm_new, d_dates, d_add, d_skip, new_blended_cids, new_manual_cids,             new_monthly_blen, new_monthly_manual = aggregate_dialer(
            combined_d_rows, pd_d, handoff_set, existing_blended_cids, existing_manual_cids,
        ...
        del ai_rows; gc.collect()  # free AI rows — largest sheet
        ...
        db_set("monthly_unique_counts", monthly_unique_counts)
        ...
        changes = {
            ...
            "Dialer Dump":    f"{d_add} calls processed ({len(b_new)} blended, {len(dm_new)} direct), {d_skip} skipped",
...
def debug():
    ...
    return jsonify({
        ...
        "migration_would_trigger": not bool(stored.get("dialer_blended_by_day")),
    })
...
# Also start keepalive when running under gunicorn
_start_keepalive()
