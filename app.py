...
import os, json, datetime, difflib, threading, time, gc, re
try:
    import psycopg2
...
# Genuine data-entry misspellings that normalisation alone cannot merge.
# Key is the lower-cased normalised form; value is the canonical name.
# 'Mansi_Negi_Jarvis' (1,755 dials) vs 'Manshi_Negi_Jarvis' (40) are one person.
AGENT_ALIASES = {
    "mansi negi": "Manshi Negi",
}

# Pipeline progression, best outcome first. A borrower who was disbursed by any
# lender is a won case regardless of how many other lenders declined, so the
# headline status must be the BEST outcome — not whichever row sorted last.
STATUS_RANK = [
    "Disbursed", "Sanctioned", "Credit Review", "Login",
    "Login Pending", "Hold", "Rejected", "Case Dropped", "Unknown",
]
...
def normalize_agent(name):
    """
    Collapse every spelling of an agent into ONE canonical name.

    The same person arrives from three sources with three spellings —
    Flexy `user` ('Nitish_upadhyay'), Flexy `supervisor_name`
    ('SugandhaGandhi_Jarvis') and Jarvis `Agent Name` ('Nitish Upadhyay').
    Because agents are keyed by this string during aggregation, any difference
    splits one person into several leaderboard rows. Normalising here — before
    aggregation — is what keeps one person to one row.
    """
    if not name:
        return ""
    s = str(name).strip()
    s = re.sub(r"[_\s]*jarvis\s*$", "", s, flags=re.I)   # drop trailing "_Jarvis"
    s = s.replace("_", " ").replace("-", " ")
    s = re.sub(r"(?<=[a-z])(?=[A-Z])", " ", s)           # SugandhaGandhi -> Sugandha Gandhi
    ...
    for p in parts[1:]:
        if p.lower() != deduped[-1].lower():
            deduped.append(p)
    out = " ".join(deduped).title()
    return AGENT_ALIASES.get(out.lower(), out)


def normalize_lender(name):
    """Trim only — casing is preserved for display; matching is done casefolded."""
    return re.sub(r"\s+", " ", str(name or "").strip())


def entity_key(borrower, lender):
    """
    Case/whitespace-insensitive identity for a pipeline entry.

    'muthoot' and 'Muthoot' are the same lender; keying on the raw string filed
    them as two separate cases for the same borrower.
    """
    return f"{normalize_lender(borrower).casefold()}|{normalize_lender(lender).casefold()}"
...
def validate_pipeline(rows):
    ...
    for i, r in enumerate(rows, 2):
        ...
        if isinstance(raw, bool):
            ...
            if not s:
                warnings.append(_w(sheet, str(i),
                    f"Status is blank for '{borrower}' — this case will NOT be counted "
                    f"in the pipeline until a status is entered"))
            else:
...
def aggregate_dialer(rows, processed_dates_by_source, handoff_set=None,
    ...
    for r in rows:
        ...
        agent    = normalize_agent(ss(r.get("user", "")))
        if agent:
            agent_sources["user"] += 1
        else:
            agent = normalize_agent(ss(r.get("supervisor_name", "")))
            if agent:
...
def process_pipeline(rows, existing_rows):
    ...
    for row in existing_rows:
        existing[entity_key(row.get("borrower", ""), row.get("lender", ""))] = row

    # Canonical display casing: 'muthoot' and 'Muthoot' are one lender, so pick a
    # single spelling (most frequent, tie-broken toward proper capitalisation)
    # and use it everywhere. Without this the Lender Summary lists both.
    def _canon_map(values):
        groups = {}
        for v in values:
            groups.setdefault(v.casefold(), []).append(v)
        return {k: sorted(vs, key=lambda x: (-vs.count(x),
                                             -sum(c.isupper() for c in x), x))[0]
                for k, vs in groups.items()}

    _lender_disp   = _canon_map([normalize_lender(ss(r.get("Lender Name", "")))
                                 for r in rows if ss(r.get("Lender Name", ""))])
    _borrower_disp = _canon_map([normalize_lender(ss(r.get("Borrower Name", "")))
                                 for r in rows if ss(r.get("Borrower Name", ""))])

    pipeline = {}
    added = updated = 0
    fuzzy_warns = []
    held = []          # rows excluded because the case has no status yet

    for r in rows:
        borrower = normalize_lender(ss(r.get("Borrower Name", "")))
        lender   = normalize_lender(ss(r.get("Lender Name", "")))
        if not borrower or not lender:
            continue
        borrower = _borrower_disp.get(borrower.casefold(), borrower)
        lender   = _lender_disp.get(lender.casefold(), lender)

        date_lead  = excel_to_date(r.get("Date of Lead"))
        label_lead = date_label(date_lead) if date_lead else ""
        key        = entity_key(borrower, lender)

        status_raw  = ss(r.get("Status", ""))
        # A case does not enter the pipeline until its status is marked. Holding
        # it out keeps counts and ₹ totals honest — an unmarked row is not yet a
        # live case. It is reported as a warning so it is never lost silently.
        if not status_raw:
            held.append(f"{borrower} / {lender}")
            continue
    ...
    # Report dropped entries by their display names, not the casefolded key
    dropped = [f"{existing[k].get('borrower','?')} / {existing[k].get('lender','?')}"
               for k in existing if k not in pipeline]
    if held:
        fuzzy_warns.append(
            f"{len(held)} case{'' if len(held)==1 else 's'} held out of the pipeline — "
            f"status not yet marked in the sheet: " + "; ".join(held[:5])
            + (" …" if len(held) > 5 else ""))
